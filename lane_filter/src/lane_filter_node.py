...
For more info on algorithm and parameters please refer to the google doc:
 https://drive.google.com/open?id=0B49dGT7ubfmSX1k5ZVN1dEU4M2M
	...
	self.d_max       = rospy.get_param("~d_max",1)
        self.d_min       = rospy.get_param("~d_min",-1)
        self.phi_min     = rospy.get_param("~phi_min",-pi)
        self.phi_max     = rospy.get_param("~phi_max",pi)
    ...
    def processSegments(self,segment_list_msg):
	    ...
	    #rospy.loginfo('progress 1 stopped here d_i %f, phi_i %f, l_i %f ',d_i,phi_i,l_i)
            #if d_i > self.d_max or d_i < self.d_min or phi_i < self.phi_min or phi_i>self.phi_max:
	#	rospy.loginfo('continue 1 ')
        #        continue
        #    if self.use_max_segment_dist and (l_i > self.max_segment_dist):
	#	rospy.loginfo('continue 2 ')
        #        continue

            #i = floor((d_i - self.d_min)/self.delta_d)
            #j = floor((phi_i - self.phi_min)/self.delta_phi)

            #rospy.loginfo('progress 2 stopped here d_i %f, phi_i %f, l_i %f, i %f, j %f',d_i,phi_i,l_i,i,j)
            ...
            #return
	np.linalg.norm(measurement_likelihood)
	...
	#rospy.loginfo('generate %f %f %f %f', segment.pixels_normalized[0].x,segment.pixels_normalized[0].y, segment.pixels_normalized[1].x,segment.pixels_normalized[1].y)
	    ...
	    rospy.loginfo('                                   WHITE d %.1f d %.1f ', d1,d2 )	
	    ...
	    rospy.loginfo('YELLOW d %.1f d %.1f ', d1,d2  )
